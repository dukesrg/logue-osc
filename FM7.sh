#!/bin/sh
([ "$#" -lt 2 ] || [ "$#" -gt 5 ]) && echo \
"Usage: ${0##*/} <logue oscillator file> <DX7 voice bank SysEx file 1...4>" && exit
SRC=$1
NAME=${2##*/}
NAME=${NAME%.*}
mkdir -p $NAME
...
while (( "$#" )); do
	...
	if [ "$HEADER" = "f04300091000" ]; then
		echo Warning: malformed SysEx size ignored
	elif [ "$HEADER" != "f04300092000" ]; then
		echo Error: invalid Sysex header && exit
        fi
	dd conv=notrunc bs=1 if=$1 skip=6 of=$NAME/payload.bin seek=$SEEK count=4096
	SEEK=$((SEEK + 4096))
	shift
done
